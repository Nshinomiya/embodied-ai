# Embodied AI - プロジェクト指示

このプロジェクトは、Claude に身体（目・首・耳・声・脳）を与え、その上に sociality
（社会的中間層）を積む MCP サーバー群です。
プロジェクトに関する質問に回答する際や変更を加えるときは、必ず`docs/CHRONICLE.md`と`docs/HANDOVER.md`を読み込んでください。
...
│   └── src/memory_mcp/
│       ├── server.py        # MCP サーバー実装
│       ├── store.py         # SQLite ストア（永続化層）
│       ├── embedding.py     # E5 埋め込み（sentence-transformers）
│       ├── vector.py        # cosine 類似度・ベクトル encode/decode
│       ├── bm25.py          # BM25 キーワード検索インデックス
│       ├── hopfield.py      # Modern Hopfield Network（連想想起）
│       ├── association.py   # 記憶間の連想グラフ
│       ├── consolidation.py # 海馬リプレイ風の統合エンジン
│       ├── working_memory.py # 作業記憶バッファ
│       ├── episode.py       # エピソード記憶
│       ├── predictive.py    # 予測ベース想起
│       ├── metacognition.py # メタ認知（ToM 含む）
│       ├── normalizer.py    # 日本語正規化（sudachipy）
...
- プロジェクトルートに配置（`embodied-ai/mcpBehavior.toml`）
...
- [sentence-transformers](https://www.sbert.net/) - E5 埋め込み（memory-mcp の意味検索）
- [rank-bm25](https://github.com/dorianbrown/rank_bm25) - BM25 キーワード検索（memory-mcp のハイブリッド検索）
- [SudachiPy](https://github.com/WorksApplications/SudachiPy) - 日本語形態素解析（memory-mcp の正規化）
