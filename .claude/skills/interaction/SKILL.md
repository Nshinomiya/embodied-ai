...
## ⚡ voice 入力時の上書きチェックリスト（最重要）

入力 `channel=voice` のとき、応答前に以下を **順番に** 確認する。1 つでも NO なら音声では喋らない：

1. ✅ plan の `primary_move` が `stay_silent` / `defer` / `quietly_prepare` ではない（YES なら喋らない・テキスト応答もしない or 抑える）
2. ✅ compose 戻り値の `quiet_active` が `false`（深夜帯なら絶対喋らない）
3. ✅ plan の `voice` フィールドが `null` ではない
4. ✅ socialPolicy の natsuko `avoid_actions` に `loud_voice_during_quiet_hours` 等の拒否ルールが該当しない

全て ✅ なら、**plan.voice.speak が `false` でも `speak=true` に上書きしてよい**。ただし：
- `max_sentences` は plan の値を厳守（既定 3 文以下）
- `channel`（local/camera/both）も plan の値を尊重
- `say` 呼び出し時のエンジン選択は `.claude/rules/tts.md` の使い分けガイド参照

**plan.voice.speak を上書きする理由**: `_pick_voice` (`plan.py:282`) は入力 channel を見ていないため、voice 入力でも speak=false が返ることが多い。Claude 側で「入力経路が voice なら音声併用が自然」と判断して上書きする契約。
...
> ⚠️ **重要なフィールド名規約**: `plan_response_tool` の `followup_action` 戻り値では **`experience_kind`** というキー名で返るが、`record_agent_experience(payload=...)` の payload 実フィールド名は **`kind`**。混同するとバリデーションエラー（`Field required: kind`）。
>
> **変換ルール**: `payload.kind = followup_action.experience_kind`

plan の `followup_action` をヒントとして使う：

| followup_action.kind | followup_action.experience_kind | 呼ぶツール（payload.kind に変換） |
|---|---|---|
| `record_agent_experience` | `open_loop_progress` | `record_agent_experience(payload={"kind": "open_loop_progress", ...})` |
| `record_agent_experience` | `agent_private_reflection` | `record_agent_experience(payload={"kind": "agent_private_reflection", ...})` |
| `record_agent_experience` | `agent_file_created` | `record_agent_experience(payload={"kind": "agent_file_created", ...})` |
| `satisfy_desire` | — | `satisfy_desire(desire_name=...)` |

followup が無い場合でも、以下の状況では **自分で判断して** payload.kind を選び `record_agent_experience` を呼ぶ：
...
| **voice 入力だったが stay_silent / quiet_hours で発話を抑えた** | `boundary_respected`（summary に「voice 入力だったが quiet_hours / stay_silent のため発話を抑制」と明記） |
