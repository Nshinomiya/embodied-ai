...
- 撮影側 PC: VSCode 編集画面、**Word Wrap OFF**、本文 14pt+。ダークテーマでも OK（Gemini Flash は色反転を読める。誤字率が高い時のみライトテーマ切替を検討）
...
### 4. OCR（OpenRouter 直叩き / Gemini Flash 主力）
...
疑義があれば **別ファミリの vision モデルで二次 OCR**（F-9 改: 元仕様の Haiku は image 非対応のため変更）：

```bash
$HELPER ocr --image "${SESSION_DIR}/page-${page}.jpg" --model google/gemini-2.5-pro
```

差分をユーザーに見せる。改善しなければ撮り直しを促す（F-6）。Anthropic / OpenAI 系の vision モデル（claude-opus-4.1, o3 等）は OpenRouter 直叩きでも persona 拒否や hallucination が確認されたため、当面は Gemini ファミリ内のグレード違いを使う。
